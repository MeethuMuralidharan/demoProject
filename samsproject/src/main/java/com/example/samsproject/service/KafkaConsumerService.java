...
*/
    @KafkaListener(topics = "test-topic" , groupId = "my-group")
    public String listenKafka(String message)  {
//        ObjectMapper objectMapper = new ObjectMapper();
//        Parent parentData = objectMapper.readValue(message, Parent.class);
        log.info("---------new Parent---------"+message);
        return message;
    }

    public Parent listenResult(String msg) throws JsonProcessingException {
        ObjectMapper objectMapper = new ObjectMapper();
        Parent parentData = objectMapper.readValue(listenKafka(msg), Parent.class);
        return parentData;
    }
    ...
    //findByparentid
    public List<Customer> getByParentId(String id) {
        return samsRepository.findByParentId(id);
    }
}
